"""Evaluate semantic query embeddings for robust pre-retrieval routing."""
...
def main() -> None:
    ...
    for seed in SEEDS:
        ...
        predictions = {
            ...
            "semantic utility": router.predict(test_queries),
    ...
    print("Semantic utility router robustness / SciFact")
    ...
    semantic_rows = [r for r in results if r.router == "semantic utility"]
    bm25_rows = [r for r in results if r.router == "always BM25"]
    wins = sum(
        semantic.mean_utility > bm25.mean_utility
        for semantic, bm25 in zip(semantic_rows, bm25_rows, strict=True)
    )
    deltas = [
        semantic.mean_utility - bm25.mean_utility
        for semantic, bm25 in zip(semantic_rows, bm25_rows, strict=True)
    ...
    print()
    print(f"semantic utility beats always-BM25 on {wins}/{len(SEEDS)} splits")
    print(f"mean semantic utility delta vs BM25: {sum(deltas)/len(deltas):+.4f}")
